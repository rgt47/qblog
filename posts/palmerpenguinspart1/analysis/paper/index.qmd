...
dataset_overview <- tibble::tribble(
  ...
  "Year Range", paste0(min(penguins_clean$year), "–", max(penguins_clean$year))
)
...
model_summary <- tibble::tribble(
  ...
  "R²", sprintf("%.3f", model_metrics$r_squared), sprintf("%.1f%% variance explained", model_metrics$r_squared * 100),
...
assumptions_check <- tibble::tribble(
  ...
  "Outliers", sprintf("%d observations >2.5 SD", nrow(outliers)), "⚠ Present",
  "Residual Std. Error", sprintf("%.1f grams", model_metrics$rmse), "✓ Acceptable"
)
...
thresholds <- tibble::tribble(
  ...
  "Normal Range", sprintf("%.0f–%.0f", low_threshold, high_threshold), "25th–75th",
