...
this rule to specific helpers, sequences the refactor into eight
...
\subsection{Phase 3: rebuild the navigation functions}

The 14 navigation functions legitimately remain in the function
tier (each calls \texttt{cd}). The current implementation,
however, has three problems that should be fixed in a single
pass:

\begin{enumerate}
  \item \textbf{Duplicated body.} The same five-token line is
        copy-pasted fourteen times with only the subpath
        varying.
  \item \textbf{Misleading error path.} Errors are written to
        \texttt{stdout} rather than \texttt{stderr}, and the
        message \texttt{`Not in ZZCOLLAB project'} fires both
        when the project root cannot be located and when the
        root \emph{is} located but the requested subpath does
        not exist; the second case is misleading.
  \item \textbf{Sentinel false positive.}
        \texttt{\_zzcollab\_root} treats any directory containing
        \texttt{DESCRIPTION} as a project root. Every R package
        in the user's tree (for example,
        \texttt{\textasciitilde/prj/sfw/01-zzlongplot/zzlongplot/}
        in its non-zzcollab incarnation) carries a
        \texttt{DESCRIPTION} file, so the helper produces a false
        positive there and the navigation shortcuts attempt to
        \texttt{cd} into a non-existent subpath. The fix is to
        require a marker that is specific to zzcollab projects.
\end{enumerate}

The \texttt{z} prefix on the shortcut names was originally
introduced to avoid collisions with single-letter names elsewhere
in the shell. A survey of the user's environment narrows the
problem: the natural one-letter mapping (first letter of the
target subdirectory) collides with the zsh \texttt{r} builtin
(re-run last command), \texttt{/usr/bin/w} (logged-in users), and
\texttt{/opt/X11/bin/x} (an XQuartz launch helper). To avoid
shadowing the high-impact \texttt{r} builtin, three shortcuts
are remapped to free letters: \texttt{r} (R/) becomes
\texttt{c}; \texttt{c} (archive/) becomes \texttt{h}; \texttt{w}
(raw\_data/) becomes \texttt{x}. The remapping resolves both
\texttt{r} and \texttt{w} and accepts a single trivial-impact
override of \texttt{/opt/X11/bin/x} (reachable as \texttt{command
x} if ever needed).

\paragraph{Sentinel choice.} A survey of the user's actual
projects clarifies which marker file reliably distinguishes a
zzcollab project from an ordinary R package. Across thirteen
zzcollab projects under \texttt{\textasciitilde/prj/sfw/}:

\begin{itemize}
  \item All thirteen carry \texttt{DESCRIPTION} (so do all R
        packages; \texttt{DESCRIPTION} alone is the source of the
        false positive).
  \item All thirteen carry an \texttt{analysis/} subdirectory; no
        ordinary R package does.
  \item About half (4 of 13) carry \texttt{zzcollab.yaml}; the
        remainder are older projects that pre-date the marker.
\end{itemize}

The combined sentinel \texttt{[[ -f zzcollab.yaml \textbar\textbar
\ -d analysis ]]} therefore matches every zzcollab project
without false-positiving any R package.

\paragraph{Action.} Replace the existing \texttt{\_zzcollab\_root}
plus 14 navigation functions with the rebuilt versions in
Appendix~A. Replace the \texttt{nav} helper with the rewritten
version in Appendix~A.3.

\paragraph{Optional follow-up.} Move the \texttt{\_zzcollab\_root}
helper to \texttt{\textasciitilde/bin/zzcollab-root} and invoke
it via \texttt{\$(zzcollab-root)}. This brings the
project-detection walk under \texttt{shellcheck} and makes it
callable from non-interactive contexts (Makefiles, cron jobs).
The 14 wrappers themselves must remain functions because each
calls \texttt{cd}.

\paragraph{Effort.} One hour. Most of the time goes to verifying
the sentinel change against every zzcollab project the user has
on disk and confirming that no script in
\texttt{\textasciitilde/bin} or \texttt{.zshrc} accidentally
references the old \texttt{z?} names.

\paragraph{Acceptance criteria.}
\begin{itemize}
  \item All 14 single-letter shortcuts behave identically to
        the corresponding old \texttt{z?} shortcuts when invoked
        from inside a zzcollab project.
  \item In an ordinary R package (e.g.\ a directory with
        \texttt{DESCRIPTION} but no \texttt{analysis/} and no
        \texttt{zzcollab.yaml}), the shortcuts return cleanly
        with `Not in a ZZCOLLAB project' on \texttt{stderr}.
        They do not silently fail or cd into a non-existent
        subpath.
  \item When the project is found but a specific subpath is
        missing, the error message names the missing path.
  \item The zsh \texttt{r} builtin and \texttt{/usr/bin/w} are
        unaffected (the natural-mapping shortcuts that would have
        shadowed them are remapped to \texttt{c} and \texttt{x}).
        \texttt{/opt/X11/bin/x} is overridden by the navigation
        wrapper but remains reachable via \texttt{command x}.
  \item \texttt{nav} prints the new single-letter table.
\end{itemize}

\subsection{Phase 4: promote microscripts to aliases}
...
\subsection{Phase 5: harden the remaining microscripts}
...
\paragraph{Effort.} One hour. Phase~6 reduces this further by
...
\subsection{Phase 6: consolidate the latest-of helpers}

Three of the scripts hardened in Phase~5 (\texttt{ss},
...
(verbatim) is given in Appendix~B.
...
\subsection{Phase 7: clean up non-executables}
...
\subsection{Phase 8: decommission the archive directory}
...
\subsection{Phase 9: naming hygiene}
...
\subsubsection*{9a. Application launchers}
...
\texttt{git grep}). After Phase~4 these become aliases; the
...
\subsubsection*{9b. The \texttt{z?} navigation family}

Folded into Phase~3. The fourteen \texttt{z?} shortcuts are
renamed to single letters as part of the navigation rebuild. See
\S{}Phase 3 and Appendix~A for the action and the resulting code.

\subsubsection*{9c. Gratuitously cryptic helpers}
...
\begin{longtable}{@{}llp{2.5in}@{}}
  ...
  \midrule
  \texttt{bb}    & edits \texttt{\textasciitilde/docs/personal/preludelist.tex} & rename to \texttt{edit-preludes}, or absorb as the Phase 4 alias \\
  ...
  \texttt{ss}    & opens latest PDF in Skim                                     & retire after Phase~6 (replaced by \texttt{latest-of} wrapper)     \\
  \texttt{vv}    & edits latest Rmd/qmd in vim                                  & retire after Phase~6                                              \\
  \texttt{zz}    & opens latest PDF in zathura                                  & retire after Phase~6                                              \\
...
\subsection{Phase 10: install a guardrail}

The refactor of phases 1--9 cleans up a decade of accreted drift.
...
and a verification pass against the post-Phase-7 toolbox).
...
\begin{longtable}{@{}lll@{}}
  ...
  3 & Rebuild navigation: collapse, tighten sentinel, drop \texttt{z} prefix     & 1\,h            \\
  4 & Promote four microscripts to aliases                                       & 0.3\,h          \\
  5 & Harden five remaining microscripts                                         & 1\,h            \\
  6 & Consolidate \texttt{ss}, \texttt{vv}, \texttt{zz} into one helper          & 0.3\,h          \\
  7 & Relocate non-executables out of \texttt{\textasciitilde/bin}               & 1\,h            \\
  8 & Decommission \texttt{\textasciitilde/bin/archive/}                         & 0.1\,h          \\
  9 & Naming hygiene (launchers, \texttt{z?} family, cryptic helpers)            & 1\,h            \\
  10 & Install \texttt{make lint} guardrail                                      & 0.5\,h          \\
  \midrule
    & \textbf{Total}                                                             & \textbf{10.2\,h} \\
  \bottomrule
\end{longtable}
...
\begin{itemize}
  ...
  \item Shell startup time decreases by an estimated 50--150\,ms
        on the audited file. The dominant contribution comes
        from removing the \texttt{gz} family.
...
\section{Rebuilt Navigation Functions (Phase 3)}

\subsection*{A.1 Tightened project-root locator}

Replace the existing \texttt{\_zzcollab\_root} with:

\begin{lstlisting}[language=bash]
# Project-root locator. Walks upward from $PWD looking for a
# zzcollab-specific marker. The combined sentinel matches every
# zzcollab project (all carry analysis/; newer ones additionally
# carry zzcollab.yaml) without false-positiving on R packages
# that carry only DESCRIPTION.
_zzcollab_root() {
    local dir="$PWD"
    while [[ "$dir" != "/" ]]; do
        if [[ -f "$dir/zzcollab.yaml" || -d "$dir/analysis" ]]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
\subsection*{A.2 Single helper plus single-letter wrappers}

Replace the 14 \texttt{z?} function definitions with one helper
and 14 thin wrappers. The wrappers are functions (not aliases)
because each calls \texttt{cd}. The natural mapping for three
letters has been remapped to avoid name collisions: \texttt{r}
(R/) is now \texttt{c}; \texttt{c} (archive/) is now \texttt{h};
\texttt{w} (raw\_data/) is now \texttt{x}. The single surviving
override is on \texttt{/opt/X11/bin/x}, reachable via
\texttt{command x} if ever needed.

\begin{lstlisting}[language=bash]
# Single helper: find the project root, then cd to a subpath.
# Routes both failure modes to stderr with distinct messages.
_zcd() {
    local subpath="$1" root
    root=$(_zzcollab_root) \
        || { print -u2 'Not in a ZZCOLLAB project'; return 1; }
    cd -- "$root/$subpath" \
        || { print -u2 "Path missing: $root/$subpath"; return 1; }
}

# Single-letter navigation wrappers.
0() { _zcd '';                          }
a() { _zcd analysis;                    }
t() { _zcd analysis/data;               }
x() { _zcd analysis/data/raw_data;      }   # overrides /opt/X11/bin/x
y() { _zcd analysis/data/derived_data;  }
s() { _zcd analysis/scripts;            }
p() { _zcd analysis/report;             }
f() { _zcd analysis/figures;            }
c() { _zcd R;                           }   # remapped from r
m() { _zcd man;                         }
e() { _zcd tests;                       }
o() { _zcd docs;                        }
h() { _zcd archive;                     }   # remapped from c
g() { _zcd vignettes;                   }
\end{lstlisting}

\subsection*{A.3 Rewritten \texttt{nav} helper}

Replace the existing \texttt{nav} function with:

\begin{lstlisting}[language=bash]
# Print the navigation cheat-sheet. Single-letter shortcuts are
# active only when $PWD is inside a zzcollab project (a directory
# at or above $PWD contains zzcollab.yaml or an analysis/
# subdirectory).
nav() {
    cat <<'EOF'
ZZCOLLAB navigation shortcuts (single-letter):

  0  -> project root
  a  -> analysis/
  t  -> analysis/data/
  x  -> analysis/data/raw_data/      (overrides /opt/X11/bin/x;
                                      use 'command x' for original)
...
Make commands (from any subdirectory):
  mr          make r (start container)
  mr test     make test
  mr <target> make <target>
EOF
}
\end{lstlisting}

\section{Replacement for ss / vv / zz (Phase 6)}
...
\hrule
\vspace{0.5em}
{\small \emph{Rendered on 2026-05-03 at 09:34 PDT.}\\
\emph{Source:}
