...
subtitle: >
  A decision framework for separating project content,
  backup pipelines, and append-only history from cloud
  sync. Picks up where post 24 (the dotfiles repository)
  left off.
...
YAML FRONT MATTER (lines 1-20)
  ...
  [x] description     — set
  [x] image           — media/images/hero.png (placeholder until generated)
...
![A workspace where a single mounted disk fans out into several local sync mechanisms, suggesting decoupling from a single cloud provider.](media/images/hero.png){.img-fluid width=80%}

*A portable dotfiles repository is only the first half of leaving Dropbox. The other half is the rest of the workflow.*

# Introduction

I did not really appreciate how much of my daily workflow depended on Dropbox until I tried to leave it. The dotfiles migration described in [post 24](https://focusonr.org/posts/24-setupdotfilesongithub/) was the obvious half: pull the configuration files out of a cloud-mounted directory, put them under git, write an installer that creates symlinks. After committing that work, every dotfile in `$HOME` was version-controlled and reproducible on a new laptop with two commands.
...
![A workspace ambiance shot suggesting layered organisation: notebook, terminal, and reference materials arranged in distinct zones.](media/images/ambiance1.png){.img-fluid}
...
A concrete example from this migration: the file `~/.claude/history.jsonl` is an append-only log of prompts entered into the Claude Code CLI. Five machines write to it through a Dropbox-synced symlink. Most days, only one machine is active and the file syncs cleanly. The day two machines write within the same minute, Dropbox cannot merge the two append streams and creates a conflict copy. Across April 2026 alone, this produced six new conflict copies on this account, each holding several hundred KB of unique prompts. The file format is not the problem; the choice to sync an append log via a whole-file cloud provider is.
...
![Editor or terminal showing the migration in progress: a directory tree being reorganised, with both old and new paths visible.](media/images/ambiance2.png){.img-fluid}

# Things to Watch Out For

The six gotchas that hurt the most during the migration. Each lists the symptom and the fix.
...
![A workspace ambiance shot suggesting completion: a laptop closed, a notebook open to a page of summary notes.](media/images/ambiance3.jpg){.img-fluid}
...
After completing this migration on a real workflow:

- New-laptop setup is `git clone … ~/dotfiles && ./install.sh && (per-project git clones)`. About 30 minutes from clean machine to working environment.
- Conflict copies of `history.jsonl` and similar files have not recurred.
- Backup pipelines source from `$PRJ_ROOT`, which can point anywhere; Dropbox is no longer load-bearing.
...
3. Append-only files are the only category that benefits from sophisticated sync; for everything else, simpler is better.
...
*Rendered on 2026-05-07 at 16:14 PDT.*<br>
