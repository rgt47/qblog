...
`power1_shiny`. `power1_shiny` is a sub-directory containing the shiny program
file `app.R ` inside our working directory named `power1_app`.
...
We can test the app locally on our workstation by runnning it with the following shell command
...
This will, in sequence, run the R
program, load the Shiny package, and launch the app in your default browser.

Figure 1  shows our Shiny app running locally in a browser on our
...
hosting the app on a (virtual) server with the goal of sharing our collaborators. There are
many ways to accomplish this. Here we'll describe how to  'spin up' a server on Amazon Web Service EC2 and in
just a few steps, through the application of Docker, R, Shiny, and Caddy
have a secure web app running.
